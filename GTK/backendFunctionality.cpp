...
rental_response attemptRental(int one_time_code, char* bike_ids) {
  ...
  char this_post[1000];
  if (one_time_code < 10){
	  sprintf(this_post, "kiosk_id=%s&bikeTag=%s&code=000%d", kiosk_id, bike_ids, one_time_code);
  } else if (one_time_code < 100) {
	  sprintf(this_post, "kiosk_id=%s&bikeTag=%s&code=00%d", kiosk_id, bike_ids, one_time_code);
  } else if(one_time_code < 1000){
...
//  ///////////////
//  	//DEBUG
//  	logText.append("backendFuncgtionality.cpp -- attemptRental()");
//  	LOG();
//  	logText.append("post being sent to verify key:");
//  	logText.append(this_post);
//  	LOG();
//  ///////////////
...
rental_response reportBicycleProblem(int bike_id, BIKE_CONDITION_CODE condition) {
  ...
  char this_post[1000];
  int i;
  sprintf(this_post, "bikeTag=A00%d&condition=%d", bike_id, condition);
