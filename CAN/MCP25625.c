...
int initializeCAN(unsigned short identifier, int spi){
	int err;
	if(spi){
		err = initSPI(); //initialize KL26's SPI peripheral
	}
	resetCAN();
	setStdId(identifier);
	setCANSpeed();
...
void setCANSpeed(){
	//vals i think should work with 100kbs
	writeReg(REG_CNF1, 0xC0);//0x01);//0xC0 - mine);//0x3F - old); cnf1 - 11000000 - page 42
	writeReg(REG_CNF2, 0xE7);//0xac);//0xDF);//0xFF); cnf2 - 11100111
	writeReg(REG_CNF3, 0x04);//0x03); //0x03);//0x87); cnf3 - 00000100
}
